...
hz_(_hz) 
{
    ...
    if(this->simEnable)
    {
        ...
        sleep(1);
        vrep_bridge_->vrepStart();
        sleep(1);
        vrep_bridge_->vrepEnableSyncMode();
        sleep(1);
        vrep_bridge_->set_exec_time(1);
    }
    else
    {
...
bool arm_controller_interface::calcGraspPose(task_assembly::door_open_planner::Request  &req,task_assembly::door_open_planner::Response &res)
{
    ...
    if(this->simEnable)
    {
        while(ros::ok())  //!door_handle_sampler_->IsGraspExist()
        {
            vrep_bridge_->read_vrep();
            vrep_bridge_->write_vrep();
            vrep_bridge_->wait();
