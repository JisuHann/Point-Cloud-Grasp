...
std::vector<geometry_msgs::Pose> handle_sampler::getSolution(unsigned int _objNum)
{
    ...
    {
        if (pcl::io::loadPCDFile<pcl::PointXYZRGB>("/home/dyros/tmp_ws/src/grasp_sampler/MODEL/Handle.pcd", *CAD_cloud) == -1) //* load the file
        {
    ...
    {
        if (pcl::io::loadPCDFile<pcl::PointXYZRGB>("/home/dyros/tmp_ws/src/grasp_sampler/MODEL/refri.pcd", *CAD_cloud) == -1) //* load the file
        {
            ROS_INFO("Couldn't read file CAD FILES \n");
            return handle_candidate;
        }
