...
void handle_sampler::cloud_cb (const sensor_msgs::PointCloud2ConstPtr& msg)
{
    ...
    // pcl::StatisticalOutlierRemoval<pcl::PointXYZRGB> sor_f;
    // sor_f.setInputCloud(roi_cloud_);
    // sor_f.setMeanK(50);
    // sor_f.setStddevMulThresh (1.0);
    // sor_f.filter(*roi_cloud_);
...
std::vector<geometry_msgs::Pose> handle_sampler::getSolution(unsigned int _objNum)
{
    ...
    copyPointCloud(*roi_cloud_, *cloud);
    ...
    case STICK:
        ...
        if (pcl::io::loadPCDFile<pcl::PointXYZRGB>("/home/dyros/tmp_ws/src/roi_extractor/src/doorsample1.pcd", *CAD_cloud) == -1) //* load the file
        {
        ...
        std::cout<<"model cloud size"<<CAD_cloud->size()<<std::endl;
    ...
    CAD_process_cloud->header.frame_id = roi_cloud_->header.frame_id;
    pcl::toPCLPointCloud2(*CAD_process_cloud, cloud_Generated);
    pcl_conversions::fromPCL(cloud_Generated, cloud_visualization_);
    cloud_visualization_.header = cloud_msgs_.header;
    ...
    // if(cloud->size() > 0)
    // {
    //     std::string save_path = "/home/dyros/tmp_ws/src/roi_extractor/src/";
    ...
    //         std::cout<<"transform rersult of Correspoding Grouping\n"<<T_icp.matrix()<<std::endl;
    //         std::cout<<"transform rersult of Correspoding Grouping\n"<<T_icp.matrix()<<std::endl;
    //         std::cout<<"transform rersult of Correspoding Grouping\n"<<T_icp.matrix()<<std::endl;
    //         std::cout<<"transform rersult of Correspoding Grouping\n"<<T_icp.matrix()<<std::endl;
    //         std::cout<<"transform rersult of Correspoding Grouping\n"<<T_icp.matrix()<<std::endl;
